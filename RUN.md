...
- `picture-description-course.html` (корінь, повністю англ.) — наскрізний курс «From Zero to ICAO Level 5» на одному фото (`img/runway-excursion-water.jpg`): 4 модулі — 5-step метод (акордеони) → worked example (model answer з Level-5 highlighter) → 10-питальний quiz → timed practice з self-assessment. Прогрес — `pdcourse1.progress`. Контент verbatim із `TASK/picture-description-course-sources/` (method-5-steps / model-answer / quiz). Без аудіо.
